...
from constants import MAX_HALF_LIFE, WINDOW, MIN_HALF_LIFE
...
def store_cointegration_results(df_market_prices):
  ...
  for index, base_market in enumerate(markets[:-1]):
    ...
    for quote_market in markets[index +1:]:
      ...
      # Log pair
      if coint_flag == 1 and half_life <= MAX_HALF_LIFE and half_life > MIN_HALF_LIFE:
        ...
        #sorted(criteria_met_pairs, key=lambda x: x["half_life"])
